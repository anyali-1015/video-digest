...
function handleFrontTabUrl(url) {
  if (!isSupportedTabUrl(url)) {
    // Panel only supports YouTube/Bilibili watch pages — close on anything else.
    window.close();
    return;
  }
...
async function checkCurrentTab() {
  try {
    // Try multiple strategies to find a supported (YouTube/Bilibili) tab
    let tab = null;
    const watchUrlPatterns = [
    ...
    if (isSupportedTabUrl(tabs[0]?.url)) {
      tab = tabs[0];
    }
    ...
    if (!tab) {
      tabs = await chrome.tabs.query({ url: watchUrlPatterns });
      if (tabs[0]) tab = tabs[0];
    }
...
function extractVideoId(url) {
  try {
    ...
    if (urlObj.hostname.includes("bilibili.com")) {
      const match = urlObj.pathname.match(/\/video\/(BV[0-9A-Za-z]+)/);
...
function showState(state) {
  ...
  if (chatToggle) {
    chatToggle.style.display = state === "results" ? "flex" : "none";
  }
  if (state !== "results") {
    closeChatPanel();
  }
...
async function showExplanation(selectedText) {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Also log this into the floating chat history, so quick "Explain"
      // lookups and free-form chat questions live in one searchable place.
      await saveChatTurn(`解释: "${selectedText}"`, result.explanation);
    } else {
...
// ============================================================
// FLOATING CHAT — ask about any word/concept, with history
// ============================================================

const CHAT_HISTORY_STORAGE_KEY = "ytd_chat_history";
const CHAT_HISTORY_MAX_ENTRIES = 300; // Across all videos, oldest evicted first
...
async function getAllChatHistory() {
  ...
  return result[CHAT_HISTORY_STORAGE_KEY] || [];
}

/**
 * Appends one Q&A turn to storage for the current video. Shared by both the
 * free-form chat panel and the "select text -> Explain" flow, so every
 * question you've asked (however you asked it) shows up in one history.
 */
async function saveChatTurn(question, answer) {
  if (!currentVideoId) return;
  ...
  if (all.length > CHAT_HISTORY_MAX_ENTRIES) {
    all.splice(0, all.length - CHAT_HISTORY_MAX_ENTRIES);
  }
  await chrome.storage.local.set({ [CHAT_HISTORY_STORAGE_KEY]: all });
}
...
async function renderChatHistory() {
  ...
  const all = await getAllChatHistory();
  const forVideo = all
    .filter((entry) => entry.videoId === currentVideoId)
    .slice(-30);
  ...
  if (forVideo.length === 0) {
    ...
    empty.textContent =
      "在这里问任何不懂的词或问题——选中字幕里的文字点「Explain」，或者直接在下面输入。";
    historyEl.appendChild(empty);
  } else {
    forVideo.forEach((entry) => {
      historyEl.appendChild(buildChatTurnElement(entry.question, entry.answer));
    });
  }
  historyEl.scrollTop = historyEl.scrollHeight;
}

function buildChatTurnElement(question, answer, { loading = false, error = false } = {}) {
  const wrap = document.createElement("div");
  wrap.className = "chat-turn";

  const q = document.createElement("div");
  q.className = "chat-bubble chat-bubble--question";
  q.textContent = question;
  wrap.appendChild(q);

  const a = document.createElement("div");
  a.className =
    "chat-bubble chat-bubble--answer" +
    (loading ? " chat-bubble--loading" : "") +
...
async function sendChatMessage() {
  ...
  // Clear the placeholder empty-state if present
  const emptyState = historyEl.querySelector(".chat-empty");
  if (emptyState) emptyState.remove();

  const turnEl = buildChatTurnElement(question, "思考中…", { loading: true });
  ...
  try {
    ...
    const answerBubble = turnEl.querySelector(".chat-bubble--answer");
    if (result.success) {
      answerBubble.textContent = result.answer;
      answerBubble.classList.remove("chat-bubble--loading");
      await saveChatTurn(question, result.answer);
    } else {
      answerBubble.textContent = result.message || result.error || "出错了，请重试。";
      answerBubble.classList.remove("chat-bubble--loading");
      answerBubble.classList.add("chat-bubble--error");
    }
  } catch (error) {
    const answerBubble = turnEl.querySelector(".chat-bubble--answer");
...
/**
 * One-click export: downloads the currently-filtered notes list
 * (either "this video" or "all videos", matching the active filter button)
...
async function exportNotes() {
  const showAll = document
    .getElementById("notesFilterAll")
    ?.classList.contains("active");
  const filterVideoId = showAll ? null : currentVideoId;
  ...
  try {
    ...
    if (result.success) notes = result.notes || [];
  } catch (error) {
    console.error("[YouTube Digest Panel] Export notes error:", error);
    return;
  }
  ...
  notes.forEach((note) => {
    const key = note.videoId || "unknown";
    if (!byVideo.has(key)) byVideo.set(key, []);
    byVideo.get(key).push(note);
  });

  let markdown = `# Notes\n\nExported ${new Date().toLocaleString()}\n\n`;
  for (const [, videoNotes] of byVideo) {
    const first = videoNotes[0];
    markdown += `## ${first.videoTitle || "Untitled video"}\n`;
    if (first.channelName) markdown += `${first.channelName}\n`;
    markdown += "\n";
    // Chronological within a video
    videoNotes
      .slice()
  ...
  const filenameLabel = showAll
    ? "all-videos"
    : sanitizeFilename(currentVideoTitle);
  downloadTextFile(markdown, `notes-${filenameLabel}.md`);
}
