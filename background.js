...
importScripts("settings.js", "bilibili-wbi.js");

/**
 * True for any watch-page URL we support (YouTube or a Bilibili video page).
 * Used everywhere we used to hardcode "youtube.com" checks.
 */
function isSupportedVideoUrl(url) {
  const value = url || "";
  return (
    value.startsWith("https://www.youtube.com") ||
    /^https:\/\/www\.bilibili\.com\/video\//.test(value)
  );
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "fetchTranscript") {
    handleFetchTranscript(message.videoId, message.platform || "youtube")
      .then(sendResponse)
      .catch((err) => sendResponse({ error: err.message }));
    return true; // Keep the message channel open for async response
  }

  if (message.action === "askChat") {
    handleAskChat(message.question, message.context, message.history, message.videoTitle)
      .then(sendResponse)
  ...
  if (message.action === "relayToContent") {
    ...
    (async () => {
      try {
        // Query specifically for supported (YouTube/Bilibili) tabs to avoid
        // side panel context issues. Try multiple query strategies.
        ...
        // If no supported tab found, try broader queries per platform
 ...
 */
async function handleFetchTranscript(videoId, platform = "youtube") {
  if (platform === "bilibili") {
    return handleFetchBilibiliTranscript(videoId);
  }
  try {
    const settings = await getSettings();
...
// ============================================================
// TRANSCRIPT FETCHING VIA BILIBILI'S OWN SUBTITLE API
// ============================================================

/**
 * Fetches the transcript for a Bilibili video from its official CC subtitles.
 *
 * Unlike YouTube, this does not go through Supadata — Bilibili isn't one of
 * its supported platforms. Instead we call Bilibili's own (unofficial, WBI-
 * signed) endpoints directly. No extra API key is needed, but only videos
 * that already have official subtitles are supported; videos without any
 * (no automatic speech-to-text fallback) return NO_TRANSCRIPT.
 *
 * @param {string} bvid - The Bilibili video ID, e.g. "BV1xx411c7mD"
 */
async function handleFetchBilibiliTranscript(bvid) {
  try {
    if (!/^BV[0-9A-Za-z]{8,12}$/.test(String(bvid || ""))) {
      return { success: false, error: "INVALID_BVID", message: "Invalid Bilibili video ID." };
    }

    // Step 1: resolve the video's cid (needed by the subtitle endpoint).
    const viewResponse = await fetch(
      `https://api.bilibili.com/x/web-interface/view?bvid=${encodeURIComponent(bvid)}`,
      { headers: { Referer: "https://www.bilibili.com/" } },
    );
    if (!viewResponse.ok) {
      throw new Error(`Bilibili view API error: ${viewResponse.status}`);
    }
    ...
    if (viewData.code !== 0 || !viewData.data) {
      return {
        success: false,
        error: "NO_TRANSCRIPT",
        message: "Could not read this Bilibili video (private, deleted, or region-locked).",
      };
    }
    const cid = viewData.data.cid;

    // Step 2: fetch the WBI-signed player info, which lists available subtitles.
    const { imgKey, subKey } = await YTD_BILIBILI_WBI.fetchWbiKeys();
    const signed = YTD_BILIBILI_WBI.signParams({ bvid, cid }, imgKey, subKey);
    ...
    if (!playerResponse.ok) {
      throw new Error(`Bilibili player API error: ${playerResponse.status}`);
    }
    ...
    if (!Array.isArray(subtitles) || subtitles.length === 0) {
      return {
        ...
        error: "NO_TRANSCRIPT",
        message:
          "This Bilibili video has no official subtitles. Automatic transcription for subtitle-free videos isn't supported yet.",
      };
    }

    // Prefer a Chinese track, then fall back to whatever's first.
    const chosen =
      subtitles.find((s) => (s.lan || "").startsWith("zh")) || subtitles[0];
    let subtitleUrl = chosen.subtitle_url || "";
    if (subtitleUrl.startsWith("//")) subtitleUrl = `https:${subtitleUrl}`;
    if (!subtitleUrl) {
      return {
        success: false,
        error: "NO_TRANSCRIPT",
        message: "Bilibili did not return a usable subtitle file for this video.",
      };
    }

    const subtitleResponse = await fetch(subtitleUrl);
    if (!subtitleResponse.ok) {
      throw new Error(`Bilibili subtitle file error: ${subtitleResponse.status}`);
    }
    ...
    if (body.length === 0) {
      return {
        success: false,
        error: "EMPTY_TRANSCRIPT",
        message: "Bilibili returned an empty subtitle file for this video.",
      };
    }
    ...
    for (const line of body) {
      const cleanText = (line.content || "").trim();
      if (!cleanText) continue;
      const startSeconds = Math.floor(Number(line.from) || 0);
      const endSeconds = Math.floor(Number(line.to) || startSeconds);
      const minutes = Math.floor(startSeconds / 60);
      const seconds = startSeconds % 60;
      const timestamp = `${minutes}:${String(seconds).padStart(2, "0")}`;
    ...
    // Bilibili subtitles are frequently unpunctuated run-on text — unlike
    // YouTube's, which normally already carry punctuation. Best-effort fix
    // it up with a cheap DeepSeek pass; if that fails (no key, rate limit,
    // etc.) we still return the raw, readable-if-imperfect transcript.
    const settings = await getSettings();
    if (settings.aiApiKey) {
      try {
        await punctuateTranscriptInPlace(transcript, "");
      } catch (punctuationError) {
        console.warn(
          "[YouTube Digest] Bilibili punctuation pass failed, using raw text:",
          punctuationError,
        );
      }
    }
    ...
    for (const entry of transcript) {
      ...
      const timestamp = `${minutes}:${String(seconds).padStart(2, "0")}`;
      transcriptTextPlain += entry.text + " ";
      transcriptTextTimestamped += `[${timestamp}] ${entry.text}\n`;
    }
    ...
    console.error("[YouTube Digest] Bilibili transcript fetch error:", error);
    return {
      success: false,
      error: "BILIBILI_FETCH_FAILED",
      message:
        "Could not fetch this video's Bilibili subtitles. Bilibili sometimes changes its internal API — try again later.",
    };
  }
}

/**
 * Restores punctuation on a transcript array IN PLACE, one batch of segments
 * at a time (DeepSeek, JSON-in/JSON-out, aligned by stable numeric id so a
 * malformed or partial response can never scramble segment order).
 *
 * Batches stay small (<=60 lines, <=2500 characters) — punctuation is a
 * cheap, short-output task, so this keeps requests fast and keeps a single
 * bad batch from wasting a huge amount of the transcript.
 */
async function punctuateTranscriptInPlace(transcript, videoTitle) {
  const MAX_LINES_PER_BATCH = 60;
  const MAX_CHARS_PER_BATCH = 2500;

  let batchStart = 0;
  while (batchStart < transcript.length) {
    let batchEnd = batchStart;
    let charCount = 0;
    while (
      batchEnd < transcript.length &&
      batchEnd - batchStart < MAX_LINES_PER_BATCH &&
      charCount < MAX_CHARS_PER_BATCH
    ) {
      charCount += (transcript[batchEnd].text || "").length;
      batchEnd++;
    }
    // Always include at least one line, even if it alone exceeds the char budget.
    if (batchEnd === batchStart) batchEnd = batchStart + 1;

    const batch = transcript.slice(batchStart, batchEnd);
    const segments = batch.map((entry, i) => ({
      id: String(batchStart + i),
      text: entry.text,
    }));

    try {
      const segmentsJson = JSON.stringify({ segments });
      const variables = { videoTitle: videoTitle || "Unknown", segmentsJson };
      ...
      const byId = new Map();
      (Array.isArray(parsed?.segments) ? parsed.segments : []).forEach((s) => {
        if (typeof s?.id === "string" && typeof s?.text === "string" && s.text.trim()) {
          byId.set(s.id, s.text.trim());
        }
      });

      batch.forEach((entry, i) => {
        const id = String(batchStart + i);
        const punctuated = byId.get(id);
        // Sanity guard: a punctuation pass should never wildly change length
        // (e.g. the model deciding to summarize instead). Reject outliers
        // and keep the original text for that line.
        if (punctuated && punctuated.length <= (entry.text.length || 0) * 2 + 20) {
          entry.text = punctuated;
        }
      });
    } catch (error) {
      // Leave this batch's text unpunctuated and move on — one failed batch
      // shouldn't block the rest of the transcript.
      console.warn("[YouTube Digest] Punctuation batch failed:", error.message);
    }
...
async function handleSaveNote(
  ...
  try {
    const canonicalVideoUrl = YTD_SETTINGS.canonicalVideoUrl(platform, videoId);
    ...
    if (!transcript) {
      const transcriptResult = await handleFetchTranscript(videoId, platform);
    ...
    // Create timestamped URL. YouTube uses "&t=Ns"; Bilibili uses "?t=N"
    // (seconds, no unit suffix, and no pre-existing query string to join).
...
// ============================================================
// FLOATING CHAT — free-form Q&A panel in the side panel
// ============================================================

/**
 * Answers a free-form question from the floating chat panel using DeepSeek.
 * Supports short multi-turn follow-ups via `history` (an array of
 * { question, answer } pairs, most recent last — the caller trims this).
 *
 * @param {string} question - The user's question
 * @param {string} context - Optional selected text / transcript snippet
 * @param {Array} history - Recent prior turns for this video, for continuity
 * @param {string} videoTitle - Current video title, for context
 */
async function handleAskChat(question, context, history, videoTitle) {
  try {
    const settings = await getSettings();
    if (!settings.aiApiKey) {
      return {
    ...
    const trimmedQuestion = String(question || "").trim().slice(0, 2000);
    if (!trimmedQuestion) {
    ...
    const messages = [
      {
        role: "system",
        content:
          "You are a helpful study assistant embedded in a browser extension side panel next to a video the user is watching. " +
          "Answer questions about unfamiliar terms, vocabulary, or concepts concisely — a few sentences unless more detail is clearly needed. " +
          "Reply in the same language the user asks in. If video context or selected text is provided, use it to ground your answer, " +
          `but you may also answer general questions unrelated to the video. Current video title: "${videoTitle || "Unknown"}".`,
      },
    ];

    (Array.isArray(history) ? history : []).slice(-6).forEach((turn) => {
      if (turn?.question) messages.push({ role: "user", content: String(turn.question).slice(0, 2000) });
      if (turn?.answer) messages.push({ role: "assistant", content: String(turn.answer).slice(0, 4000) });
    });
    ...
    console.error("[YouTube Digest] Ask chat error:", error);
    if (error.status === 429) {
      return { success: false, error: "RATE_LIMITED", message: "DeepSeek rate-limited this request. Try again shortly." };
    }
    return { success: false, error: error.message || "Failed to get an answer" };
  }
}
